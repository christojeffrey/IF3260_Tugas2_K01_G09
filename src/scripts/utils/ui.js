...
function setupFileListener(state, defaultState) {
  ...
  function importData() {
    ...
    reader.onload = function (e) {
      ...
      for (let i = 0; i < data.position.length; i += 9) {
        let vec1 = v3.create(
          data.position[i + 3] - data.position[i],
          data.position[i + 4] - data.position[i + 1],
          data.position[i + 5] - data.position[i + 2]
        );
        let vec2 = v3.create(
          data.position[i + 6] - data.position[i],
          data.position[i + 7] - data.position[i + 1],
          data.position[i + 8] - data.position[i + 2]
        );
  ...
  function exportData() {
    ...
    for (let i = 0; i < state.object.position.length; i += 3) {
      let vec             = v3.create(
        state.object.position[i    ], 
        state.object.position[i + 1], 
        state.object.position[i + 2]
      );
      let transformedVec  = m4.multiplyWithV3(
        m4.transform(state.translation, state.rotation, state.scale), 
        vec
      );
      transformedVec         = transformedVec.slice(0, 3);
      transformedPosition    = [...transformedPosition, 
        Math.round(transformedVec[0]), 
        Math.round(transformedVec[1]), 
        Math.round(transformedVec[2])
      ];
    }
